...
class ALIF(nn.Module):
    ...
    def params_init(self, hidden_size, device):
        ...
        tau_adp_init = self.tau_adp_init * torch.ones(hidden_size).to(device)

        if self.adaptive_tau_adp:
            tau_adp = torch.nn.Parameter(tau_adp_init)
            self.register_parameter('tau_adp',tau_adp)
            torch.nn.init.normal_(self.tau_adp, mean=self.adaptive_tau_adp_mean, std=self.adaptive_tau_adp_std)
        else:
            self.register_buffer("tau_adp", tau_adp_init)
    ...
    def neuron_update(self,
        ...
        rho = torch.exp(-1. * 1. / torch.abs(self.tau_adp))
        b = rho * b + (1 - rho) * z
        # B = b_j0 + beta * b
        ...
        theta_t = theta + b * beta

        # membrane potential update
        u = u*(alpha) + x*(1.0 - alpha)-z*theta_t

        # generate spike.
        # z = StepDoubleGaussianGrad.apply(u - theta_t)
        z = ActFun_adp.apply(u - theta_t)
        
        # reset membrane potential.
        # soft reset (keeps remaining membrane potential)
        return z, u, b, theta_t
    ...
    def forward(self,x):
        ...
        mem_post = []
        self.reset()
        b = torch.ones_like(x[0,0]) * DEFAULT_ALIF_THETA
        u = torch.rand_like(x[0])#* DEFAULT_ALIF_THETA * 2.8 #*0.0
        spike = torch.zeros_like(x[0])
        for t in range(x.shape[0]):
            if self.recurrent:
                r_x = self.w(spike)
                spike, u, b, theta_t = self.neuron_update(x[t]+r_x,
                                            z=spike,
                                            u=u,
                                            b=b,
                                            )
            else:
                spike, u, b, theta_t = self.neuron_update(x[t],
                                            z=spike,
                                            u=u,
                                            b=b,
                                            )
        ...
        # mem_mask = torch.ones_like(mem_post)
        # N = 10
        # mem_mask[:N] = mem_mask[:N]*0.0
        # mem_post = mem_post*mem_mask        
