...
class OutputHook(list):
    def __init__(self, output_type='linear'):
        self.mask = 0  
    ...
    def __call__(self, module, inputs, output):
        if self.output_type=='connection':
            ...
            loss = output
            self.append(loss)        
        elif self.output_type=='wireless':
            ...
            if self.output_type == 'connection':
                if 'conv' in module.__class__.__name__.lower() or 'linear' in module.__class__.__name__.lower():
                    if 'layer' not in module.__class__.__name__.lower():
                        ...
                        self.k+=1
            elif self.output_type == 'neuron':
                if hasattr(module, self.output_type) and module.hook_enable:
                    self.module_dict[str(self.k)] = module
                    self.k+=1
            elif self.output_type == 'linear':
                if 'conv' in module.__class__.__name__.lower() or 'linear' in module.__class__.__name__.lower():
